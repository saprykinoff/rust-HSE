...
fn test_read_json_empty() {
    let mut stream = get_stream_with_message(r#"{}"#);
...
fn test_read_json_publisher_post() {
    let mut stream = get_stream_with_message(r#"{"message": "Hello 1 from Rust publisher!"}"#);
    let result = read_json(&mut stream).unwrap();
    assert!(result.method.is_none());
    assert!(result.topic.is_none());
    assert_eq!(
        result.message,
        Some(String::from("Hello 1 from Rust publisher!"))
    );
}
